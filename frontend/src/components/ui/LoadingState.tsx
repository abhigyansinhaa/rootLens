...
function SkeletonCard({ wide }: { wide?: boolean }) {
  return (
    <div className="rounded-lg border border-(--border-subtle) bg-(--surface-1) p-5 space-y-3 overflow-hidden">
...
export function LoadingState({ rows = 3, message, fullPage = false }: LoadingStateProps) {
    ...
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-6">
        ...
        <div className="relative h-12 w-12">
          ...
          <div
            className="absolute inset-2 rounded-full border border-transparent border-t-(--brand) opacity-40 animate-spin"
            style={{ animationDuration: '0.7s' }}
          />
        </div>
        {message && (
          <p className="text-sm font-medium text-(--text-3) animate-pulse">
            {message}
          </p>
        )}
      </div>
  ...
  return (
    <div className="space-y-4 animate-fade-in" role="status" aria-label={message ?? 'Loading…'}>
      ...
      {message && (
        <p className="text-center text-xs text-(--text-3) animate-pulse pt-2">{message}</p>
      )}
    </div>
  )
}
